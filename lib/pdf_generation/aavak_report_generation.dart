...
class AavakReport {
  ...
  Map<String, Map<int, Combination>> groupByDate(
      List<MilkCollectionModel> milkCollections) {
    Map<String, Map<int, Combination>> dateSeparation = {};
    ...
    for (var milk in milkCollections) {
      ...
      combination.totals[1] += milk.amount;
  ...
  List<pw.Widget> buildTable(
      Map<String, Map<int, Combination>> list) {
        ...
        widgets.add(pw.Center(
          child: pw.Column(children: [
            pw.Row(
                ...
                children: [
                  pw.Text("${entry.key}   ${customerCollection.key==0 ? 'गाय':'म्हैस'}",
                      style: pw.TextStyle(fontWeight: FontWeight.bold, fontSize: 18,font: ttf))
                ]),
            pw.SizedBox(height: 10,),
            pw.Row(
                mainAxisAlignment: pw.MainAxisAlignment.spaceAround,
                children: [
                  pw.Text("एकूण दुध : ${customerCollection.value.totals[0].toStringAsFixed(2)}",  style: pw.TextStyle(
                    font: ttf,)),
                  pw.Text("एकूण किंमत : ${customerCollection.value.totals[1].toStringAsFixed(2)}",style: TextStyle(font: ttf))
                ]),
            pw.SizedBox(height: 10,),
  ...
  pw.Widget buildInvoice(MapEntry<String, List<MilkCollectionModel>> resolvedData) {
              ...
              children: [
                pw.TableRow(children: [
                  ...
                  pw.Container(
                    alignment: pw.Alignment.center,
                    padding: pw.EdgeInsets.all(3),
                    child: pw.Text(findCustomerName(resolvedData.key)!,style: TextStyle(font: ttf)), // Name
                  ),
                ]),
              ],
            ),
                      ...
                      for (var time in [0, 1])
                        (() {
                          var milk = resolvedData.value.firstWhere(
                            ...
                            orElse: () => MilkCollectionModel(customerId: "", adminId: "", fat: 0, snf: 0, milkType: 0, time: 0, date: DateTime.now(), rate: 0, amount: 0, quantity: 0),
                          );
                          return [
                            pw.Container(
                              height: 20,
                            ...
                            pw.Container(
                              height: 20,
                              alignment: pw.Alignment.center,
                              child: pw.Text((milk.amount??'-').toString(), style: pw.TextStyle(fontSize: 9)),
                            ),
                          ];
                        })()
                    ].expand((e) => e).toList(),
